...
if(isset($_POST['submit'])){
    ...
    // if(!empty($_POST['titre']) && !empty($_POST['duree']) && !empty($_POST['synopsis']) && !empty($_POST['genre']) && !empty($_POST['affiche']) && !empty($_POST['images']) && !empty($_POST['trailer'])){
        ...
        $taille_max = 1000000; // Taille maximale (1Mo)
        ...
        if($affiche['error'] == UPLOAD_ERR_OK){
            ...
            if(in_array($extension_upload, $extensions_valides)){
                ...
                if($affiche['size'] <= $taille_max){
                    ...
                    $nom_nouveau_fichier = $titre_film . '_' . uniqid() . '.' . $extension_upload;
        ...
        // Insertion des données de catégorisation dans la table Catégoriser
        foreach($categories as $id_categorie){
            $req_ins = $connex->prepare('INSERT INTO Categoriser (id_film, id_categorie) VALUES (?, ?)');
            $req_ins->execute([$id_film, $id_categorie]);
        }
                ...
                if($images['error'][$key] == UPLOAD_ERR_OK){
                    ...
                    if(in_array($extension_upload, $extensions_valides)){
                        ...
                        if($images['size'][$key] <= $taille_max){
                            // L'upload du fichier est OK
                            $nom_nouvelle_image = $titre_film . '_' . uniqid() . '.' . $extension_upload;
        ...
        if($trailer['error'] == UPLOAD_ERR_OK){
            ...
            if(in_array($extension_upload, $extensions_valides)){
                ...
                if($trailer['size'] <= $taille_max){
                    // L'upload du fichier est OK
                    $nom_nouvelle_video = $titre_film . '_' . uniqid() . '.' . $extension_upload;
