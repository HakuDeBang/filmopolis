...
<div id="myTabContent">
    ...
    <div class="hidden mx-10 rounded-lg bg-eerieBlack" id="films" role="tabpanel" aria-labelledby="films-tab">
        <form class="flex flex-row mb-4 justify-end">
            ...
            <a href="#" type="button" data-modal-target="addFilmModal" data-modal-show="addFilmModal" class="text-antiflashWhite bg-eerieBlack border border-antiflashWhite font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Ajouter un film</a>
        </form>
                        ...
                        <td class="px-4 py-4">
                            <div class="xl:grid xl:grid-cols-2 place-items-center gap-2">
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-2">
                                    <?php
                                        $req_sel = $connex->prepare('SELECT * FROM Images WHERE id_film = ?');
                                        $req_sel->execute([$film['id_film']]);
                                        $images = $req_sel->fetchAll();
                                        foreach($images as $image){?>
                                        <div>
                                            <img class="w-96 md:rounded-lg" src="<?= $image['lien_images']; ?>" alt="">
                                        </div>
                                    <?php } ?>
                                </div>
                            </div>
